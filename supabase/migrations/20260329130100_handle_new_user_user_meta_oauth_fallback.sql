-- Fallback OAuth (Apple/Google) desde raw_user_meta_data cuando raw_app_meta_data->>'provider'
-- no viene en el insert (p. ej. algunos flujos signInWithIdToken Apple).
-- No aplica a invitaciones staff/admin (misma condición que v_is_staff_invite).
...
BEGIN
  ...
  -- OAuth: provider en app metadata (sin cambios)
  ...
  -- Fallback: user_meta (provider / signup_method) solo si app_metadata no definió google/apple
  -- y no es flujo de invitación staff/admin.
  IF COALESCE(v_provider, '') NOT IN ('google', 'apple')
     AND NOT (
  ...
  THEN
    v_um_provider := NULLIF(TRIM(COALESCE(NEW.raw_user_meta_data->>'provider', '')), '');
    v_um_signup_method := NULLIF(TRIM(LOWER(COALESCE(NEW.raw_user_meta_data->>'signup_method', ''))), '');
    IF v_um_signup_method IN ('google', 'apple') THEN
      v_signup_method := v_um_signup_method;
    ELSIF v_um_provider = 'google' THEN
      v_signup_method := 'google';
    ELSIF v_um_provider = 'apple' THEN
      v_signup_method := 'apple';
    END IF;
  END IF;
...
COMMENT ON FUNCTION public.handle_new_user() IS
  'Trigger auth.users insert. OAuth: app_metadata.provider; fallback user_meta provider/signup_method si no google/apple en app (excl. invites).';

COMMENT ON COLUMN public.users.signup_method IS
  'password | google | apple | admin_invite. handle_new_user (app_metadata + user_meta fallback) o Edge ensure-owner-oauth-metadata.';
