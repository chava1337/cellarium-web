-- Fix: staff invite por signup_intent/intent ya no cae como owner.
-- Añade v_is_staff_invite (invitationType = admin_invite OR signup_intent = staff_invite OR intent = staff_invite).
-- Nombre: name → username → split_part(email,'@',1).
-- No rehace onboarding; cambio mínimo compatible.
...
COMMENT ON FUNCTION public.handle_new_user() IS
  'Trigger after insert auth.users: insert public.users. Staff si invitationType=admin_invite o signup_intent/intent=staff_invite; si no, owner. Nombre: name → username → email local part.';
