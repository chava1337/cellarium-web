-- Observabilidad temporal en handle_new_user: v_step y RAISE EXCEPTION con contexto para localizar fallo en signup OAuth owner.
-- No revierte: v_branch_id := NULL para owner, detección Google por raw_app_meta_data->>'provider'. No toca staff invite.
...
COMMENT ON FUNCTION public.handle_new_user() IS
  'Trigger after insert auth.users. Instrumentado: v_step y RAISE EXCEPTION con contexto en fallos (insert_public_user, create_default_branch, assign_default_branch). Quitar observabilidad tras diagnosticar.';
