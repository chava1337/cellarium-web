-- Fix: v_is_staff_invite puede ser NULL cuando invitationType/signup_intent/intent son NULL (Google OAuth).
-- NULL hace que "IF NOT v_is_staff_invite" no entre y owner_id quede NULL → viola users_owner_owner_id_required.
-- Solución: asignación null-safe y bloque owner explícito COALESCE(v_is_staff_invite, false) = false.
...
BEGIN
  ...
  -- Detección Google OAuth: provider en app metadata
...
COMMENT ON FUNCTION public.handle_new_user() IS
  'Trigger after insert auth.users. v_is_staff_invite null-safe (COALESCE). Owner: owner_id=NEW.id, branch_id=NULL luego Sucursal Principal. Google vía provider. Observabilidad: step en EXCEPTION.';
